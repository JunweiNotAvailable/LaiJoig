        ...
        {options.map((option, i) => {
          return (
            <Pressable key={option.route}>
              <Button style={[styles.option]} textStyle={{ fontSize: 16, }} icon={option.icon} text={option.name} onPress={() => option.onPress ? option.onPress() : navigation.navigate(option.route)}/>
            </Pressable>
          )
        })}
      </ScrollView>
